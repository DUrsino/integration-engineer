...
const calculateAverageScore = (matches) => {
  return Number((matches.reduce((acc, m) => acc + m.score, 0) / matches.length).toFixed(3));
};
...
export const logger = {
  chat: {
    query: (ip, browser, query, matches, responseChars, timeMs, responseBody) => {
  ...
  system: {
    docsLoad: (files, totalBytes, timeMs) => baseLogFormat.formatLog('system', 'docs_load', {
      docs: {
        files: files.map(f => ({ 
          name: f.filename, 
          bytes: f.chars,
    ...
    chunksCreate: (chunks, avgSize, fileCount, timeMs) => baseLogFormat.formatLog('system', 'chunks_create', {
      chunks: {
        count: chunks,
        avg_size: avgSize,
